...
//   const { userInfo } = useContext(VariableContext);
  ...
  async function getShortSummary(){
    ...
    toast.success('Wait for few seconds')
      if(pdfContent && selectedPdf){
          try {
              const response = await axios.post(`https://ai-v2-backend.onrender.com/pdfSummary`, { pdfContent });
  ...
  async function getQuestions(){
    ...
    setPdfQuestions('')
      if(pdfContent && selectedPdf){
          try {
              const response = await axios.post(`https://ai-v2-backend.onrender.com/pdfQuestions`, { pdfContent });
  ...
  const handleEnter = async (e) => {
    ...
    //   }
      ...
      if(question){
        try {
          const response = await axios.post(`https://ai-v2-backend.onrender.com/askDoubt`, { pdfContent , question });
